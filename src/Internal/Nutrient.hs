...
ingredientToTree
  ...
  -> Mass
  -> MappedFoodItem
  -> (FinalGroupedFood, NutrientMap)
ingredientToTree ms mass f =
  first
    (bimap (scaleNV scale) (scaleNV $ Energy $ unMass scale))
    $ foodItemToTree
    $ foldr modifyItem f ms
  where
    scale = mass / 100
...
foodItemToTree :: MappedFoodItem -> (FinalGroupedFood, NutrientMap)
foodItemToTree (FoodItem i d ns cc pc) =
  first go $ runState (displayTree pc) ns
  where
    go t = bimap toNV toNV $ FinalFood t $ computeCalories cc t
    toNV :: a -> NutrientValue_ (Sum a)
